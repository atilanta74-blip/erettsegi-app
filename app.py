...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 8, 'Magyar Irodalom Erettsegi Tetelvazlatok', border=False, align='C')
        self.ln(8)
...
def letoltheto_pdf_generalas(tetelek_adat):
    ...
    for cim, adat in tetelek_adat.items():
        ...
        pdf.cell(0, 6, f"{cim} - {adat['alcim']}".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.ln(2)
        
        pdf.set_font('Helvetica', '', 8.5)
        tiszta_vazlat = adat['vazlat'].replace('###', '').replace('**', '').replace('*', '-')
        for sor in tiszta_vazlat.strip().split('\n'):
            pdf.multi_cell(0, 4.2, sor.strip().encode('latin-1', 'replace').decode('latin-1'))
        pdf.ln(4)
